...
class MWBot {
  ...
  constructor(customOptions, customRequestOptions) {
     ...
     * @type {Object}
     */
    this.globalRequestOptions = MWBot.merge(
      this.defaultRequestOptions,
  ...
  setGlobalRequestOptions(customRequestOptions) {
    ...
    this.customRequestOptions = MWBot.merge(
      this.customRequestOptions,
      customRequestOptions
    );
  }
  ...
  login(loginOptions) {
    this.loginPromise = new Promise((resolve, reject) => {
      ...
      let loginString =
        this.options.username +
        "@" +
        this.options.apiUrl.split("/api.php").join("");
        ...
        .then((response) => {
          if (!response.login || !response.login.result) {
            ...
            if (!this.options.silent)
              log(
                "[E] [MWBOT] Login failed with invalid response: " + loginString
              );
            return reject(err);
          } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise[operation](
          ...
          return this[operation](pageName, job[2], job[3], job[4])
            .then((response) => {
              ...
              if (operation === "delete") {
                ...
                response.upload &&
                response.upload.result === "Warning"
              ) {
                status = "[/] ";
                if (
                  response.upload.warnings &&
                  response.upload.warnings.duplicate
                ) {
                  reason = "duplicate";
                  debugMessages.push(
                    "[D] [MWBOT] Duplicate: " +
                      response.upload.warnings.duplicate.join(", ")
                  );
                }
                if (
                  response.upload.warnings &&
                  response.upload.warnings.exists
                ) {
                  reason = "exists";
                  debugMessages.push(
                    "[D] [MWBOT] Exists: " + response.upload.warnings.exists
                  );
                }
              }
        ...
        .catch((err) => {
          ...
          if (!this.options.silent) {
            log(
              "[E] [MWBOT] At least one exception occured during the batch job:"
            );
            log(err);
          }
